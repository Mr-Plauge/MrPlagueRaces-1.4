	...
	public class FortunatePlayer : ModPlayer
	{
		...
		public override void PreUpdate() {
			if (usingItem == false && animating == true) {
				for (int i = 0; i < 3 * (int)(ModContent.GetInstance<GoblinConfig>().goblinPrefixes.ContainsKey(GoblinPrefixType.Fortunate) ? (1f + StatConfigHelpers.RacialStatPercentageFloatIndex[(int)ModContent.GetInstance<GoblinConfig>().goblinPrefixes[GoblinPrefixType.Fortunate]]) : 1f); i++)
				{
