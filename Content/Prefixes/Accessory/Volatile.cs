...
namespace MrPlagueRaces.Content.Prefixes
{
	...
	public class VolatilePlayer : ModPlayer
	{
		...
		public override void PostHurt(Player.HurtInfo info)
		{
			...
			if (volatileFactor > 0) {
				Projectile.NewProjectile(Wiring.GetProjectileSource(0, 0), Player.Center.X + Main.rand.Next(15) - Main.rand.Next(15), Player.Center.Y + Main.rand.Next(15) - Main.rand.Next(15), 0, 0, ProjectileType<VolatileExplosion>(), ((Player.statLifeMax2 / 25) * volatileFactor) * (int)(ModContent.GetInstance<GoblinConfig>().goblinPrefixes.ContainsKey(GoblinPrefixType.Volatile) ? (1f + StatConfigHelpers.RacialStatPercentageFloatIndex[(int)ModContent.GetInstance<GoblinConfig>().goblinPrefixes[GoblinPrefixType.Volatile]]) : 1f), 5, Player.whoAmI);
			}
		}
	}
}